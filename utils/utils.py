...
class losses_saver():
    def __init__(self, opt):
        self.name_list = ["Generator_D_body", "Generator_D_cloth", "Generator_D_densepose", "Generator_CD", "Generator_PD", "Vgg", "L1", "LPIPS", "D_fake_body", "D_fake_cloth", "D_fake_densepose", "D_real_body", "D_real_cloth", "D_real_densepose", "LabelMix", "CD_fake", "CD_real", "PD_fake", "PD_real"]
        self.opt = opt
    ...
    if condition_run_stats:
        with torch.no_grad():
            ...
            for i, data_i in enumerate(dataloader):
                ...
                fake = model.module.netEMA(image["I_m"], image["C_t"], label["body_seg"], label["cloth_seg"], label["densepose_seg"], agnostic=agnostic)
...
class image_saver():
    def __init__(self, opt):
        ...
        self.opt = opt
        self.num_cl = {"body_seg": opt.semantic_nc[0] + 1, "cloth_seg": opt.semantic_nc[1] + 1, "densepose_seg": opt.semantic_nc[2] + 1}
    ...
    def visualize_batch(self, model, image, label, cur_iter, agnostic=None):
        ...
        with torch.no_grad():
            model.eval()
            fake = model.module.netG(image["I_m"], image["C_t"], label["body_seg"], label["cloth_seg"], label["densepose_seg"], agnostic=agnostic)
